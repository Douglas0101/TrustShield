...
class AdvancedLogger:
    ...
    def __init__(self, name: str):
        ...
        if not self.logger.handlers:
            ...
            formatter = logging.Formatter('%(asctime)s - [TrustShield-Evaluator] - %(levelname)s - %(message)s')
...
class EvaluationEvent(Enum):
    ...
    EVALUATION_START, MODEL_LOADING_START, MODEL_LOADING_COMPLETE, EVALUATION_TYPE_START, \
    EVALUATION_TYPE_COMPLETE, REPORT_GENERATED, EVALUATION_COMPLETE, EVALUATION_FAILED = range(8)
...
class EvaluationStrategy(Protocol):
    ...
    def evaluate(self, model: BaseEstimator, X: pd.DataFrame, y: pd.Series, config: Dict[str, Any]) -> EvaluationResult: ...
    def generate_report(self, results: List[EvaluationResult], output_path: str) -> None: ...
...
class PerformanceEvaluationStrategy(BaseEvaluationStrategy, EvaluationStrategy):
    ...
    def evaluate(self, model: BaseEstimator, X: pd.DataFrame, y: pd.Series, config: Dict[str, Any]) -> EvaluationResult:
        ...
        return EvaluationResult(EvaluationType.PERFORMANCE, "placeholder_model", "v1", True, 0.95, {}, 1.5)
...
class EvaluationStrategyFactory:
    ...
    @staticmethod
    def create_strategy(evaluation_type: EvaluationType, config: Dict[str, Any], logger: AdvancedLogger, monitor: ResourceMonitor) -> EvaluationStrategy:
        strategies = {
        ...
        if not strategy_class:
            raise ValueError(f"Estratégia de avaliação não encontrada para o tipo: {evaluation_type.value}")
...
class ResilientModelEvaluator(Subject):
    ...
    def run_evaluation(self, evaluation_types_str: List[str] = None):
        ...
        try:
            ...
            if evaluation_types_str is None:
                evaluation_types_str = self.config.get('evaluation', {}).get('enabled_types', ['performance'])
            evaluation_types = [EvaluationType(et) for et in evaluation_types_str]

            all_results = {}
            for eval_type in evaluation_types:
                self.notify(EvaluationEvent.EVALUATION_TYPE_START, {"evaluation_type": eval_type})
                strategy = EvaluationStrategyFactory.create_strategy(eval_type, self.config, self.logger, self.monitor)

                # Condição robusta para execução paralela
                if RAY_AVAILABLE and ray and self.config.get('evaluation', {}).get('parallel_processing', False):
                    results = self._run_parallel_evaluation(strategy, models, X, y)
                else:
                    self.logger.log(logging.INFO, "Executando avaliação sequencial (Ray não disponível ou desativado).")
                    results = self._run_sequential_evaluation(strategy, models, X, y)

                all_results[eval_type.value] = results
                output_dir = self.project_root / "outputs" / "evaluation" / eval_type.value
                strategy.generate_report(results, str(output_dir))
                self.notify(EvaluationEvent.EVALUATION_TYPE_COMPLETE, {"evaluation_type": eval_type})

            self._generate_consolidated_report(all_results)
            self.notify(EvaluationEvent.EVALUATION_COMPLETE, {"total_time": time.time() - start_time})
    ...
    def _run_sequential_evaluation(self, strategy: EvaluationStrategy, models: Dict[str, Tuple[BaseEstimator, str]], X: pd.DataFrame, y: pd.Series) -> List[EvaluationResult]:
    ...
    def _run_parallel_evaluation(self, strategy: EvaluationStrategy, models: Dict[str, Tuple[BaseEstimator, str]], X: pd.DataFrame, y: pd.Series) -> List[EvaluationResult]:
        """Distribui a avaliação dos modelos em paralelo usando Ray."""
        if not RAY_AVAILABLE or not ray:
            self.logger.log(logging.ERROR, "Tentativa de execução paralela sem Ray. Retornando para sequencial.")
        ...
        if not ray.is_initialized():
            ray.init(num_cpus=self.config.get('evaluation', {}).get('max_workers', psutil.cpu_count(logical=False)))

        # A função da tarefa remota é definida aqui dentro para garantir que 'ray' não seja 'None'.
        @ray.remote
        def _evaluate_task(strategy_obj: EvaluationStrategy, model_obj: BaseEstimator, x_df: pd.DataFrame, y_series: pd.Series,
                           config_dict: Dict[str, Any], model_name_str: str, model_version_str: str) -> EvaluationResult:
        ...
        futures = [
            _evaluate_task.remote(strategy_ref, ray.put(model), X_ref, y_ref, config_ref, model_name, model_version)
            for model_name, (model, model_version) in models.items()
        ]
    ...
    def _load_data(self) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        full_path = self.data_path if self.data_path.is_absolute() else self.project_root / self.data_path
        ...
        if 'is_anomaly' in data.columns:
            ...
            self.logger.log(logging.WARNING, "Coluna 'is_anomaly' não encontrada. Gerando labels sintéticos com IsolationForest.")
    ...
    def _load_models(self) -> Dict[str, Tuple[BaseEstimator, str]]:
        ...
        for model_path in self.model_paths:
            full_path = model_path if model_path.is_absolute() else self.project_root / model_path
            self.notify(EvaluationEvent.MODEL_LOADING_START, {"model_path": str(full_path)})
            artifact = joblib.load(full_path)
            model = artifact.get('model') if isinstance(artifact, dict) else artifact
            version = artifact.get('training_timestamp', 'legacy') if isinstance(artifact, dict) else 'legacy'
            name = full_path.stem
            models[name] = (model, version)
            self.notify(EvaluationEvent.MODEL_LOADING_COMPLETE, {"model_name": name})
        return models

    def _generate_consolidated_report(self, all_results: Dict[str, List[EvaluationResult]]):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sistema de Avaliação de Modelos TrustShield")
    parser.add_argument("--data", type=str, required=True, help="Caminho para os dados de avaliação (.parquet)")
    parser.add_argument("--models", type=str, nargs='+', required=True, help="Lista de caminhos para os modelos (.joblib)")
    parser.add_argument("--types", type=str, nargs='+', default=['performance'], help="Tipos de avaliação a serem executados (ex: performance robustness)")
    parser.add_argument("--config", type=str, default="config/config.yaml", help="Caminho para o arquivo de configuração principal")
    ...
    try:
        ...
        try:
            ...
            logger.log(logging.CRITICAL, f"Erro fatal no pipeline de avaliação: {e}", exc_info=True)
        except:
