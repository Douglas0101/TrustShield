...
class InterpretationEvent(Enum):
    (
        ...
        INTERPRETATION_METHOD_START,
        INTERPRETATION_METHOD_COMPLETE,
        RESULTS_SAVED,
        REPORT_GENERATED,
...
class Subject:
    def __init__(self):
        self._observers: List[InterpretationObserver] = []

    def attach(self, observer: InterpretationObserver):
...
class InterpretationStrategy(Protocol):
    def interpret(
        self, model: Any, data: pd.DataFrame, config: Dict[str, Any]
    ) -> Dict[str, Any]: ...
...
class MLflowObserver(InterpretationObserver):
    def __init__(self, experiment_name: str, project_root: Path):
        self.experiment_name, self.run_id, self.project_root = (
            experiment_name,
            None,
            project_root,
        )
    ...
    def update(self, event: InterpretationEvent, data: Dict[str, Any]):
        if event == InterpretationEvent.INTERPRETATION_START:
            mlflow.start_run(
                run_name=f"interpretation_{datetime.now().strftime('%Y%m%d-%H%M%S')}",
            ...
            if results_path := data.get("results_path"):
                mlflow.log_artifact(results_path)
        elif event == InterpretationEvent.REPORT_GENERATED:
            if report_path := data.get("report_path"):
...
class BaseInterpretationStrategy:
    ...
    def _sample_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        return (
            data.sample(n=max_samples, random_state=42)
            if len(data) > max_samples
            else data
        )

    def _calculate_metrics(
        self, start_time: float, samples: int, features: int
...
class ShapInterpreter(BaseInterpretationStrategy, InterpretationStrategy):
    def interpret(
        ...
        if not SHAP_AVAILABLE:
            raise ImportError(
                "SHAP library is not installed. Please install it to use this feature."
            )
...
class InterpretationStrategyFactory:
    ...
    @staticmethod
    def create_strategy(
        method: InterpretationMethod, config: Dict[str, Any], logger: AdvancedLogger
        ...
        if not strategy_class:
            raise ValueError(
                f"Estratégia de interpretação não encontrada para o método: {method.value}"
            )
...
class ResilientModelInterpreter(Subject):
    ...
    def _load_model(self) -> Any:
        try:
            ...
            if not model:
                raise ValueError("Artefato de modelo inválido")
            self.logger.log(logging.INFO, f"Modelo carregado: {type(model).__name__}")
            return model
        except Exception as e:
            self.logger.log(
                logging.ERROR, f"Falha ao carregar modelo: {e}", exc_info=True
            )
            raise
    ...
    def run_interpretation(self, data_path: str, methods: List[str] = None):
        ...
        try:
            self.notify(
                InterpretationEvent.INTERPRETATION_START,
                {
            ...
            self.notify(
                InterpretationEvent.DATA_VALIDATION_COMPLETE,
                {"validation_status": "success", "samples": len(data)},
            )

            if methods is None:
                methods = self.config.get("interpretability", {}).get(
                    "methods", ["shap"]
                )

            interpretation_methods = [
                InterpretationMethod(m)
                for m in methods
                if m in InterpretationMethod._value2member_map_
            ]

            results = {}
            for method in interpretation_methods:
                self.notify(
                    InterpretationEvent.INTERPRETATION_METHOD_START, {"method": method}
                )
                strategy = InterpretationStrategyFactory.create_strategy(
                    method, self.config, self.logger
                )
                interpretation_results = strategy.interpret(
                    self.model, data, self.config
                )
                output_dir = (
                    self.project_root / "outputs" / "interpretations" / method.value
                )
                strategy.save_results(interpretation_results, str(output_dir))
                results[method.value] = interpretation_results
                self.notify(
                    InterpretationEvent.INTERPRETATION_METHOD_COMPLETE,
                    {
                        "method": method,
                        "metrics": interpretation_results["metrics"],
            ...
            self.notify(
                InterpretationEvent.REPORT_GENERATED, {"report_path": str(report_path)}
            )

            self.notify(
                InterpretationEvent.INTERPRETATION_COMPLETE,
                {
                    "total_time": time.time() - start_time,
                    "methods_used": [m.value for m in interpretation_methods],
                },
            )
    ...
    def _load_and_validate_data(self, data_path: str) -> pd.DataFrame:
        try:
            ...
            self.logger.log(
                logging.INFO, f"Dados carregados e validados: {len(data)} amostras"
            )
            return data
        except Exception as e:
            self.logger.log(
                logging.ERROR, f"Falha ao carregar dados: {e}", exc_info=True
            )
            raise
    ...
    def _generate_consolidated_report(self, results: Dict[str, Any]) -> Path:
        ...
        report_path = (
            report_dir
            / f"interpretation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sistema de Interpretação de Modelos TrustShield"
    )
    parser.add_argument(
        "--model", type=str, required=True, help="Caminho para o modelo"
    )
    ...
    try:
        interpreter = ResilientModelInterpreter(
